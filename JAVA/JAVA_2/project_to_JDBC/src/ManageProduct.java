...
public class ManageProduct extends JFrame implements ActionListener{//관리자 모드
	...
	private Object[]newArr=null; int code, rowNum=0, stockNum=0;
	DefaultTableModel model;
	Goods goods=new Goods();//객체 생성
	Connection connection = null;	Statement stmt = null;	ResultSet rs=null;	PreparedStatement prepared=null;
	public void closeFrame() {
		this.setVisible(false);
	}
	public ManageProduct(Connection conn, Statement stmt, ResultSet rs, PreparedStatement pst){
		connection=conn;
		stmt=stmt;
		rs=rs;
		prepared=pst;
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					System.out.println("물품이 등록되었습니다.");
					{
						...
						Vector row=new Vector();
						row.add(code);row.add((String)comboBox.getSelectedItem());row.add(name.getText());
		...
		del_button.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				try {
					...
					model.removeRow(row);
					//데이터베이스 삭제
					JOptionPane.showMessageDialog(null,"데이터를 삭제하겠습니다.");//알림판
					String sql="delete from goodsinfo where code=?";//삭제 명령문
					prepared=connection.prepareStatement(sql);
					prepared.setInt(1, row);
					prepared.execute();
				}catch(Exception e2) {
