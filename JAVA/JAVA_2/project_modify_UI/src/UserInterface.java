			...
			try {
				...
				case 1://사용자 메뉴
					if(manager.getManageSize()==0) {//등록된 물품이 없을 때
						...
						try {
							answer = scan.nextInt();scan.nextLine();//키보드 버퍼 해결
							if(answer==1) {//상품 구매를 선택한 경우
								int index=0;//구매 물품 인덱스
								int buy=0;//살지(1)/말지(0)
								int sellCount=0;//구매 수량
								System.out.println("##사용자 물품 구매##");
								System.out.println("##전체 물품 목록 출력##");
								for(int i=0;i<manager.getManageSize();i++) {//수정 가능한 번호 알려주기
									System.out.println("["+(i+1)+"번] 물품 카테고리:"+ manager.goodsArr(i).getCategory());
			                        System.out.println("["+(i+1)+"번] 물품명 :"+manager.goodsArr(i).getName());
			                        System.out.println("["+(i+1)+"번] 물품 가격 :"+manager.goodsArr(i).getPrice());
			                        System.out.println("["+(i+1)+"번] 물품 재고 :"+manager.goodsArr(i).getStock());
			                        System.out.println("-----------------------------");
								}
								try {
									...
									System.out.println("["+(index+1)+"번] 물품 카테고리:"+ manager.goodsArr(index).getCategory());
			                        System.out.println("["+(index+1)+"번] 물품명 :"+manager.goodsArr(index).getName());
			                        System.out.println("["+(index+1)+"번] 물품 가격 :"+manager.goodsArr(index).getPrice());
			                        System.out.println("["+(index+1)+"번] 물품 재고 :"+manager.goodsArr(index).getStock());
									try {// 카테고리로 물품 찾기
				    					System.out.printf("물품을 구매하시겠습니까?(구매하면1, 구매안하면0):");
					...
					Menu.manageMenu();//관리자 메뉴를 보여준다.
							try {
								...
								case 1://물품 등록
									...
									System.out.println("##관리자 물품 등록##");//물품 등록							
									try {
										//등록할 물품 정보 입력
										...
										price=scan.nextInt();scan.nextLine();//키보드 버퍼 해결
										System.out.printf("물품 개수를 입력하세요:");
										stock=scan.nextInt();scan.nextLine();//키보드 버퍼 해결
									...
									if(manager.getManageSize()==0) {//물품이 하나도 없을 때
										...
										}
										{
											try {
												...
												int change_p=scan.nextInt();scan.nextLine();
												System.out.printf("수정할 물품 개수를 입력하세요:");
												int change_s=scan.nextInt();scan.nextLine();
								...
								case 4://삭제
									...
									if(manager.getManageSize()==0||manager.getManageSize()<0) {//물품이 없을 때
										...
										break;
											...
											try {
												System.out.printf("물품을 삭제하시겠습니까?(삭제하면1, 삭제 안하면0):");
								...
								case 5://물품 검색
									...
									if(manager.getManageSize()==0||manager.getManageSize()<0) {//물품이 없을 때
										...
										try {
					                        ...
					                        System.out.println("["+(selectGoodsIndex+1)+"번] 물품 카테고리:"+ manager.goodsArr(selectGoodsIndex).getCategory());
					                        System.out.println("["+(selectGoodsIndex+1)+"번] 물품명 :"+manager.goodsArr(selectGoodsIndex).getName());
					                        System.out.println("["+(selectGoodsIndex+1)+"번] 물품 가격 :"+manager.goodsArr(selectGoodsIndex).getPrice());
					                        System.out.println("["+(selectGoodsIndex+1)+"번] 물품 재고 :"+manager.goodsArr(selectGoodsIndex).getStock());
